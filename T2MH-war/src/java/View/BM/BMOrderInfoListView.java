 ...
 * @author 19319
 */
@Named(value = "bMOrderInfoListView")
@ManagedBean
@ViewScoped
public class BMOrderInfoListView implements Serializable {
    ...
    public void init() {
        ...
        branchF.refresh();
        //get all product => get orderCollection
        List<Product> listProduct = (List<Product>) bm.getBranchID().getProductCollection();
        List<OrderInfo> listOrderInfo = new ArrayList<>();
        for (Product product : listProduct) {
            List<OrderInfo> listOrderOfProduct = (List<OrderInfo>) product.getOrderInfoCollection();
            for (Iterator<OrderInfo> iterator = listOrderOfProduct.iterator(); iterator.hasNext();) {
                OrderInfo next = iterator.next();
                listOrderInfo.add(next);
...
//                                if ((fieldValue.equalsIgnoreCase("Fail") || fieldValue.equalsIgnoreCase("Admin Cancel") || fieldValue.equalsIgnoreCase("Refuse")) && filterValue.equals(fieldValue)) {
//                                    match = true;
//                                } else {
                                if (filterValue == null || fieldValue.startsWith(filterValue.toString().toLowerCase())) {
                                    match = true;
            ...
            if (sortField != null) {
                try {
                    if (sortField.equalsIgnoreCase("timeOrder")) {
                        Collections.sort(data, new BMOrderInfoListView.DateTimeLazySorter(sortField, sortOrder));
                    } else {
                        Collections.sort(data, new BMOrderInfoListView.LazySorter(sortField, sortOrder));
                    }
                    ...
                    if (sortField.equalsIgnoreCase("productID") || sortField.equalsIgnoreCase("orderId")) {//get inner class=pk
                        ...
                        Collections.sort(listOIPK, new BMOrderInfoListView.LazySorterInverseFields(sortField, sortOrder));
